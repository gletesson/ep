...
const createMobileMenu = () => {
  ...
  mobileMenuDOM.addEventListener("click", (event) => {
    event.stopPropagation(); //pour éviter que le menu se ferme si on clique dessus
  });
...
iconMobile.addEventListener("click", (event) => {
  event.stopPropagation(); // pour empêcher la propagation de l’événement à window
  toggleMobileMenu();
});

// fermeture menu lorsqu'on clique sur window
